...
def test_lerp_preserves_endpoints_and_interpolates_alpha() raises:
    ...
    assert_true(start.lerp(end, 0.0).equals(start))
    assert_true(start.lerp(end, 1.0).equals(end))
...
def test_public_operations_revalidate_mutated_storage() raises:
    ...
    with assert_raises(contains="red must be finite"):
        _ = corrupted.red()
    with assert_raises(contains="red must be finite"):
        _ = corrupted.lerp(RGBA.black(), 0.5)

    var invalid_alpha = RGBA.black()
    invalid_alpha._alpha = 2.0
    with assert_raises(contains="alpha must be finite"):
        _ = invalid_alpha.alpha()
    with assert_raises(contains="alpha must be finite"):
        _ = RGBA.black().equals(invalid_alpha)
